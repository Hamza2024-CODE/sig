...
<div class="portal-container">
    ...
    <div class="portal-mobile-nav">
        @foreach ($navItems as $item)
            @if ($item !== null)
                <a href="{{ url('portal/' . $item['key']) }}"
                   class="portal-mobile-link {{ $page === $item['key'] ? 'active' : '' }}">
        ...
        <div class="col-lg-4 col-xl-3 d-none d-lg-block">
            <div class="portal-sidebar-card">
                ...
                @foreach ($navItems as $item)
                    ...
                    @else
                        <a href="{{ url('portal/' . $item['key']) }}"
                           class="portal-sidebar-link {{ $page === $item['key'] ? 'active' : '' }}">
                            <i class="fa-solid {{ $item['icon'] }}"></i>
                            {{ $item['label'] }}
                        </a>
                    @endif
        ...
        <div class="col-lg-8 col-xl-9">
            <div class="portal-content-glass animate__animated animate__fadeIn">
                ...
                <div class="portal-page-header">
                    <h2 class="portal-title-gradient">{{ $title }}</h2>
                </div>

                <div class="portal-rich-text">
                    @if (isset($dfeps))
                        <script>
                            window.portalDfeps = @json($dfeps);
                        </script>
                    @endif
