...
@section('title', 'البريد الداخلي | SGFEP')
    ...
    <div class="d-flex align-items-center gap-3 mb-4 flex-wrap">
        ...
        <div>
            <h1 class="fw-black m-0" style="font-size:1.25rem;font-family:'Cairo';color:var(--tx-1);">
                البريد الداخلي
                @if($unread > 0)
                <span class="unread-pill ms-2">{{ $unread }}</span>
                @endif
            </h1>
    ...
    <div class="msg-layout">
        ...
        <aside class="msg-sidebar">
            <div class="glass-panel p-3">
                ...
                <nav>
                    <a href="?filter=inbox" class="msg-nav-item {{ $filter==='inbox' ? 'active' : '' }}">
                        <i class="fa-solid fa-inbox"></i>
                        الوارد
                        @if($unread > 0)<span class="unread-pill">{{ $unread }}</span>@endif
                    </a>
                    <a href="?filter=sent" class="msg-nav-item {{ $filter==='sent' ? 'active' : '' }}">
                        <i class="fa-solid fa-paper-plane"></i>
                        المرسَل
                    </a>
                    <a href="?filter=broadcast" class="msg-nav-item {{ $filter==='broadcast' ? 'active' : '' }}">
            ...
            <div class="glass-panel p-0 overflow-hidden">
                ...
                <div class="d-flex align-items-center justify-content-between px-4 py-3" style="border-bottom:1px solid var(--border);">
                    <h6 class="fw-black mb-0 d-flex align-items-center gap-2" style="font-family:'Cairo';font-size:.92rem;color:var(--tx-1);">
                        ...
                        @endif
                        <span style="font-size:.78rem;font-weight:600;color:var(--tx-3);">({{ $messages->total() }} رسالة)</span>
                    </h6>
                </div>
                ...
                <a href="{{ route('messages.show', $msg->id) }}" class="msg-item {{ !$msg->is_read ? 'unread' : '' }}">
                    <div class="msg-avatar {{ $msg->channel==='broadcast' ? 'broadcast' : '' }}">
                        @if($msg->channel==='broadcast')
                            <i class="fa-solid fa-bullhorn" style="font-size:.8rem;"></i>
                        @else
                            {{ mb_strtoupper(mb_substr($msg->subject ?: 'ر', 0, 1)) }}
                        @endif
                    </div>
                    <div class="msg-content">
                        <div class="msg-meta">
                            <span class="msg-sender">
                                ...
                                @elseif($filter==='sent') إلى: #{{ $msg->receiver_id }}
                                @else من: #{{ $msg->sender_id }} @endif
                            </span>
                            <span class="msg-time">{{ $msg->created_at->diffForHumans() }}</span>
                        </div>
                        <div class="msg-subject">{{ $msg->subject ?: '(بدون موضوع)' }}</div>
                        <div class="msg-preview">{{ Str::limit(strip_tags($msg->body), 75) }}</div>
                    </div>
                ...
                <div class="d-flex justify-content-center gap-2 p-3">
                    ...
                    <a href="{{ $messages->previousPageUrl() }}" class="btn btn-sm" style="background:var(--bg-glass);border:1px solid var(--border);color:var(--tx-2);font-family:'Cairo';">
                        <i class="fa-solid fa-chevron-right"></i> السابق
                    </a>
                    @endif
                    @if($messages->hasMorePages())
                    <a href="{{ $messages->nextPageUrl() }}" class="btn btn-sm" style="background:var(--bg-glass);border:1px solid var(--border);color:var(--tx-2);font-family:'Cairo';">
            ...
            <div class="row g-3">
                <div class="col-12 col-md-6">
                    ...
                    <select name="channel" class="cp-input" id="channelSel" onchange="toggleReceiver()">
                        ...
                        @if(in_array($role, ['admin','drh','directeur','dfep','high_admin']))
