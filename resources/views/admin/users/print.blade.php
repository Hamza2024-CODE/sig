...
<div class="container my-5 p-5 bg-white border shadow-sm" style="max-width: 950px; font-family: 'Cairo', sans-serif; position: relative;">
    ...
    <div class="row mb-4 text-end small text-muted">
        <div class="col-6">
            <span>الرقم: <strong>SGFEP/DFEP/USER/<?= date('Y') ?>/<?= rand(100, 999) ?></strong></span>
        </div>
    ...
    <div class="text-center mb-4 py-3 border border-dark border-2 bg-light">
        <h3 class="fw-bold m-0" style="color: #1e293b; font-size: 1.4rem;">سجل مستخدمي المنصة وتفويضات الصلاحيات البيداغوجية</h3>
        <span class="text-muted fw-bold d-block mt-1">Registre des Utilisateurs & Habilitations de Sécurité</span>
    </div>
    ...
    <div class="table-responsive mb-5">
        <table class="table table-bordered align-middle text-right" style="border: 1px solid #1e293b;">
            <thead class="bg-light text-dark fw-bold" style="border-bottom: 2px solid #1e293b;">
                <tr>
                    <th class="py-2">الاسم الكامل / الاسم المستعار</th>
                    <th class="py-2 text-center">الرتبة والدور</th>
                    <th class="py-2">المؤسسة / الجهة المنتسبة</th>
                    <th class="py-2">البريد الإلكتروني</th>
                    <th class="py-2 text-center">الصلاحيات الإدارية</th>
                </tr>
            </thead>
            <tbody>
                <?php if (empty($users)): ?>
                    <tr>
                        <td colspan="5" class="text-center py-4">لا يوجد مستخدمون مسجلون حالياً.</td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($users as $u): ?>
                        <?php 
                            // Parse permissions to display as readable text
                            $permsArr = [];
                            if (!empty($u['permissions'])) {
                                $perms = json_decode($u['permissions'], true);
                                if (is_array($perms)) {
                                    foreach ($perms as $pKey => $val) {
                                        if ($val == 1) {
                                            $permsArr[] = $pKey;
                                        }
                                    }
                            ...
                            $permsStr = empty($permsArr) ? 'افتراضية / الأساسية' : implode('، ', $permsArr);
                        ?>
                        <tr>
                            <td>
                                <strong><?= htmlspecialchars($u['nom_complet']) ?></strong>
                                <div class="text-muted small" style="font-family: 'Outfit'; font-size: 0.75rem;">@<?= htmlspecialchars($u['username']) ?></div>
                            </td>
                            <td class="text-center">
                                <span class="badge bg-secondary text-white px-2 py-1"><?= htmlspecialchars($u['role_ar']) ?></span>
                            </td>
                            <td>
                                <span class="small"><?= htmlspecialchars($u['etab_nom'] ?? 'المديرية الولائية (DFEP)') ?></span>
                            </td>
                            <td style="font-family: 'Outfit'; font-size: 0.85rem;"><?= htmlspecialchars($u['email']) ?></td>
                            <td class="small text-muted text-center" style="font-size: 0.78rem;">
                                <?= htmlspecialchars($permsStr) ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
    ...
    <div class="row mt-5 pt-4 text-center">
        ...
        <div class="col-6 text-start">
            <span class="fw-bold d-block mb-5">مصلحة الموارد البشرية والوسائل</span>
    ...
    <div class="mt-5 pt-4 border-top text-center no-print">
        <button onclick="window.print()" class="btn btn-dark px-4 py-2" style="border-radius: 8px;">
            <i class="fa-solid fa-print me-1"></i> بدء طباعة القائمة
        </button>
