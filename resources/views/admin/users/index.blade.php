...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (editWilaya) {
        editWilaya.addEventListener('change', function() {
            ...
            autoGenerateDfepUsername('edit');
        });
    }

    const addDept = document.getElementById('department_id');
    if (addDept) {
...
const dfepDepts = [
    { value: '', label: 'بدون مصلحة (مدير المديرية الولائية / دور عام)' },
    { value: 'sdtpp', label: 'مصلحة التكوين الحضوري (البيداغوجيا)' },
    { value: 'sdtpa', label: 'مصلحة التمهين (Apprentissage)' },
    { value: 'sdtpc', label: 'مصلحة التكوين المتواصل والشراكة' },
    { value: 'dplm', label: 'مصلحة الامتحانات والشهادات' },
    { value: 'biao', label: 'مكتب التوجيه والإعلام (Orientation)' },
    { value: 'admfin', label: 'مصلحة المالية والوسائل' },
    { value: 'sdarh', label: 'مصلحة الموارد البشرية' }
];

function populateDeptSelectForDfep(deptSelect) {
...
function restoreDefaultDeptSelect(deptSelect) {
    if (!deptSelect) return;
    deptSelect.innerHTML = '<option value="">بدون مديرية (أو دور خارجي)</option>';
}

function autoGenerateDfepUsername(mode) {
    ...
    if (roleCode === 'dfep') {
        ...
        const wilayaSelect = document.getElementById(mode === 'add' ? 'wilaya_id' : 'edit_wilaya_id');
        const usernameInput = document.getElementById(mode === 'add' ? 'username' : 'edit_username');
        
        if (deptSelect && wilayaSelect && usernameInput) {
            const deptCode = deptSelect.value;
            const wilayaId = wilayaSelect.value;
            
            const currentVal = usernameInput.value.trim();
            const isDefaultOrEmpty = currentVal === "" 
                || currentVal.startsWith('sdtp') 
                || currentVal.startsWith('dplm') 
                || currentVal.startsWith('biao') 
                || currentVal.startsWith('admfin') 
                || currentVal.startsWith('sdarh') 
                || currentVal.startsWith('dfep');
                
            if (isDefaultOrEmpty) {
                if (deptCode) {
                    const suffix = wilayaId ? `_${wilayaId}` : '';
                    usernameInput.value = deptCode + suffix;
                } else {
                    usernameInput.value = wilayaId ? `dfep_${wilayaId}` : '';
                }
            }
        }
    }
...
function handleRoleChange(roleId, mode) {
    ...
    if (roleCode === 'admin') {
        // Global system admin: no central department, no CFPA/INSFP school
        if (deptSelect) {
            restoreDefaultDeptSelect(deptSelect);
            deptSelect.value = '';
        }
    ...
    } else if (roleCode === 'dfep') {
        // Regional DFEP: show DFEP departments list, hide CFPA/INSFP school
        if (deptSelect) {
            populateDeptSelectForDfep(deptSelect);
        }
    ...
    // Auto-generate username for DFEP departments if applicable
    autoGenerateDfepUsername(mode);
}
...
function editUser(user) {
    ...
    let deptVal = user.department_id || '';
    if (user.role_code === 'dfep') {
        const usernameLower = (user.username || '').toLowerCase();
        if (usernameLower.startsWith('sdtpp') || usernameLower.startsWith('sdtpc') || usernameLower.startsWith('pedago')) {
            deptVal = 'sdtpp';
        } else if (usernameLower.startsWith('sdtpa')) {
            deptVal = 'sdtpa';
        } else if (usernameLower.startsWith('dplm')) {
            deptVal = 'dplm';
        } else if (usernameLower.startsWith('biao')) {
            deptVal = 'biao';
        } else if (usernameLower.startsWith('admfin') || usernameLower.startsWith('samf')) {
            deptVal = 'admfin';
        } else if (usernameLower.startsWith('sdarh') || usernameLower.startsWith('samrh')) {
            deptVal = 'sdarh';
        }
    }
    document.getElementById('edit_department_id').value = deptVal;
