...
function applyEtabFilters() {
    ...
    rows.forEach(row => {
        ...
        if (rangeVal === "has_trainees") {
            matchesRange = rowTotal > 0;
        } else if (rangeVal === "no_trainees") {
            matchesRange = rowTotal === 0;
        } else if (rangeVal === "high") {
            matchesRange = rowTotal > 100;
        }

        if (matchesSearch && matchesWilaya && matchesType && matchesRange) {
            row.style.display = "";
            visibleEtabCount++;
            totalSum += rowTotal;
            femmesSum += rowFemmes;
    ...
    // Update Counts Card elements dynamically
    ...
    // Show no results row if list is empty
...
<?php echo $__env->make('layouts.main', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\sig\resources\views/admin/modules/reconduits.blade.php ENDPATH**/ ?>